...
public class FilesCreator {
	...
	//Checks to make sure that the file/directory specified both exists and is readable
	public static void checkExistsAndReadable(File inFile){
		if(!inFile.exists()){
			System.out.println("ERROR: "+ inFile.getName() + " does not exist");
			System.exit(1);
		} else if (!inFile.canRead()){
			System.out.println("ERROR: " + inFile.getName() + " can not be read");
			System.exit(1);
		}
	}
	
	//Given a file, returns an ArrayList containing a single MusicFile, representing the input file
	...
	//Given a directory, returns an ArrayList containing MusicFiles representing the files inside the given directory
	public static ArrayList<MusicFile> makeDirectoryList(File dir){
		...
		if (files.length == 0){
			// One directory is empty, there are no comparisons to do, so we can immediately exit
			System.exit(0);
		}
	...
	public static MusicFile makeMusicFile(File file){
		...
		if(fileName.endsWith(".wav")){
			...
			System.out.println("ERROR: "+ file.getName() + " is not a supported format");
			System.exit(1);
		}
