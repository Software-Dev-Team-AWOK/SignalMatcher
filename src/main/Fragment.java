...
public class Fragment {
	...
	public static void main(String[] args) {
		...
		Fragment fragment3 = new Fragment(Arrays.copyOfRange(list2, 0, 5), 
				random);
	...
	public boolean equals(Object o) {
		...
		for (int i = 0; i<num_substrings; i++) {
			...
			System.out.println(Arrays.toString(Arrays.copyOfRange( input.list,
					i, i+this.length()))+ "with hash value : " + 
					new Fragment(Arrays.copyOfRange( input.list, i, 
							i+this.length()),constants).hashCode());

			hash.put(new Fragment(Arrays.copyOfRange( input.list, i,
					i+this.length()),constants).hashCode() % num_substrings, 
					new Fragment(Arrays.copyOfRange( input.list, i, 
							i+this.length()),constants));
		}
